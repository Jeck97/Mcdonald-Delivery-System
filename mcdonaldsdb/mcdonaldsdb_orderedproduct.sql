...
CREATE TABLE `orderedproduct` (
  `OrderedProductId` int NOT NULL AUTO_INCREMENT COMMENT 'Identification of ordered product record.',
  `Order` int NOT NULL COMMENT 'Reference to the order number.',
  `Product` int NOT NULL COMMENT 'The ordered product.',
  `Quantity` int NOT NULL COMMENT 'Quantity of the ordered product',
  `Subtotal` double NOT NULL COMMENT 'Subtotal of the ordered product',
  PRIMARY KEY (`OrderedProductId`),
  KEY `Order_idx` (`Order`),
  KEY `Product_idx` (`Product`),
  CONSTRAINT `Order` FOREIGN KEY (`Order`) REFERENCES `order` (`OrderId`),
  CONSTRAINT `Product` FOREIGN KEY (`Product`) REFERENCES `product` (`ProductId`)
